...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b-4 border-amber-500 bg-slate-950 text-white">
      {/* Üst Bilgi Barı */}
      ...
      <div className="container mx-auto flex items-center justify-between gap-5 px-4 py-4">
        <Link href="/" className="shrink-0 font-heading text-2xl font-black">
          İzmir <span className="text-amber-400">Çekici</span>
          <span className="block text-[10px] tracking-[2px] text-slate-300">
            OTO KURTARMA • YOL YARDIM
          </span>
        </Link>
        ...
        <nav className="relative hidden items-center gap-5 whitespace-nowrap text-[11px] font-extrabold xl:flex">
          ...
          <Link href="/hizmet-bolgeleri" className="hover:text-amber-400 transition">
            TÜM BÖLGELER
          </Link>
        ...
        <a
          ...
          className="hidden shrink-0 bg-amber-500 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-amber-400 sm:flex"
        >
        ...
        <button
          type="button"
          className="grid h-11 w-11 place-items-center border border-slate-700 bg-slate-900 xl:hidden"
          onClick={() => setO(true)}
      ...
      >
        ...
        <nav
          ...
          className={`absolute right-0 top-0 flex h-dvh w-[min(90vw,430px)] flex-col bg-slate-950 shadow-2xl transition-transform duration-300 ease-out ${
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-800 px-6 py-5">
            <div>
              ...
              <p className="mt-1 text-[10px] font-bold tracking-[.16em] text-slate-400">7/24 YOL YARDIM</p>
            </div>
            <button
              type="button"
              onClick={close}
              aria-label="Menüyü kapat"
              className="grid h-10 w-10 place-items-center border border-slate-700"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <Link
              href="/"
              onClick={close}
              className="block border-b border-slate-800 py-4 text-sm font-extrabold"
            >
              ANA SAYFA
            </Link>
            ...
            {S.map((x) => (
              <details key={x.slug} className="border-b border-slate-800">
                ...
                <div className="grid gap-2 pb-4 pl-3 text-sm text-slate-300">
          ...
          <a
            href={`tel:${MAIN_PHONE_RAW}`}
            className="m-6 flex items-center justify-center gap-2 bg-amber-500 px-4 py-4 text-sm font-extrabold text-slate-950"
          >
